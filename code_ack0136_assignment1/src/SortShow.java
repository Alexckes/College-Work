...
public class SortShow extends JPanel { 
		...
		public void SelectionSort(){
			...
			for(int x = 0; x < total_number_of_lines; x++){
				int smallest = getIndexOfSmallest(x,total_number_of_lines);
				swap(x,smallest);
		...
		public int getIndexOfSmallest(int first, int last){
			...
			//I completed this part
			int small = lines_lengths[first];
			int smallInd = first;
			for(int i = first + 1; i < last; i++){
				if(small > lines_lengths[i]){
					small = lines_lengths[i];
					smallInd = i;
				}
			}
		...
		public void BubbleSort(){
			...
			for(int a = 0; a < total_number_of_lines - 1; a++)
			{
				for(int b = 0; b < total_number_of_lines - a - 1; b++)
				{
					if(lines_lengths[b] > lines_lengths[b+1])
					{
						swap(b,b+1);
	...
	public void quickSort() {
		//getting the date and time when the recursive merge sort starts
		Calendar start = Calendar.getInstance();
			recursiveQuickSort(lines_lengths, 0, total_number_of_lines - 1);
		Calendar end = Calendar.getInstance();
		//getting the time it took for the iterative merge sort to execute
		//subtracting the end time with the start time
	...
	public void recursiveQuickSort(int[] a, int low, int hi)
	{
		//redrawing the lines_lengths
		paintComponent(this.getGraphics());
		//Causing a delay for 10ms
		...
		recursiveQuickSort(a, low, split-1);
		recursiveQuickSort(a, split+1, hi);
	}
	...
	///////////////////////////////////////////////////////////////////////////////////
		...
		public void R_MergeSort(int first, int last){
			if(first < last){
				...
				//You need to complete this part.
				int mid = first + (last-first)/2; //divide
				R_MergeSort(first,mid); //recur
				R_MergeSort(mid+1,last); //recur
				R_Merge(first,mid,last); //conquer
		...
		public void R_Merge(int first, int mid, int last){
			...
			for(int i = 0; i < n1; i++)
				L[i] = lines_lengths[first + i];
			for(int j = 0; j < n2; j++)
				R[j] = lines_lengths[mid + 1 + j];
			//Merge time
			int i = 0, j = 0;
			int k = first;
			while (i < n1 && j < n2){
